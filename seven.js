...
let items = [5, 6, -5, 5, 3, 5, 3, -2, 0]
console.log(findMaxLenSubarr(items, 8))

function findMaxLenSubarr(items, sum)
{
	let localSums = []	// keeps track of all local sums
	let leftRight = [0, 0]	// start and end index of longest sequence producing sum
	...
	*/
	...
	{
		localSums.push(0)	// increases the number of local sums being tracked
		for (let ind = 0; ind < localSums.length; ind++)
		{
			localSums[ind] += val
			/*
			...
			*/
			if (localSums[ind] == sum && localSums.length - ind > leftRight[1] - leftRight[0])
			{
				leftRight = [ind, localSums.length]
			}
		}
	}

	return items.slice(leftRight[0], leftRight[1])
}